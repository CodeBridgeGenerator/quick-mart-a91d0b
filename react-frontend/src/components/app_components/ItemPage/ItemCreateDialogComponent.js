...
const ItemCreateDialogComponent = (props) => {
    ...
    const validate = () => {
        ...
        const error = {};
            ...
            if (_.isEmpty(_entity?.supplierId)) {
                error["supplierId"] = `Supplier ID field is required`;
                ret = false;
            }
  
            if (_.isEmpty(_entity?.supermarketId)) {
                error["supermarketId"] = `Supermarket ID field is required`;
                ret = false;
            }
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            name: _entity?.name,description: _entity?.description,price: _entity?.price,stockqty: _entity?.stockqty,categoryId: _entity?.categoryId,supplierId: _entity?.supplierId,supermarketId: _entity?.supermarketId,itemId: _entity?.itemId,image: _entity?.image,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Item created successfully" });
        props.onCreateResult(result);
        } catch (error) {
            console.log("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Item" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Item" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="item-create-dialog-component">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputNumber id="stockqty" className="w-full mb-3 p-inputtext-sm" value={_entity?.stockqty} onChange={(e) => setValByKey("stockqty", e.value)}  />
            </span>
            <small className="p-error">
            {!_.isEmpty(error["stockqty"]) ? (
              <p className="m-0" key="error-stockqty">
                {error["stockqty"]}
              </p>
            ) : null}
          </small>
...
export default connect(mapState, mapDispatch)(ItemCreateDialogComponent);
