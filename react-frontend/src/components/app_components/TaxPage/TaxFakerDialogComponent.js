...
import taxFakerFactory from "./taxFakerFactory";
...
const TaxFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = taxFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("tax").create(elem));
...
export default connect(mapState, mapDispatch)(TaxFakerDialogComponent);
