...
import SupportTicketDatatable from "./SupportTicketDataTable";
import SupportTicketEditDialogComponent from "./SupportTicketEditDialogComponent";
import SupportTicketCreateDialogComponent from "./SupportTicketCreateDialogComponent";
import SupportTicketFakerDialogComponent from "./SupportTicketFakerDialogComponent";
import SupportTicketSeederDialogComponent from "./SupportTicketSeederDialogComponent";
...
const SupportTicketPage = (props) => {
    ...
    const filename = "supportTicket.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("supportTicket");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "SupportTicket", type: "error", message: error.message || "Failed get SupportTicket" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("supportTicket").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "SupportTicket", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("supportTicket").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/supportTicket/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="supportTicket-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="supportTicket-datatable">
                    <SupportTicketDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <SupportTicketEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <SupportTicketCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <SupportTicketFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <SupportTicketSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(SupportTicketPage);
