...
import supportTicketFakerFactory from "./supportTicketFakerFactory";
...
const SupportTicketFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = supportTicketFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("supportTicket").create(elem));
...
export default connect(mapState, mapDispatch)(SupportTicketFakerDialogComponent);
