...
import couponFakerFactory from "./couponFakerFactory";
...
const CouponFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = couponFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("coupon").create(elem));
...
export default connect(mapState, mapDispatch)(CouponFakerDialogComponent);
